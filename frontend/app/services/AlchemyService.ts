...
export class AlchemyService {
  ...
  async getBalance(address: string, network: 'ethereum' | 'polygon' = 'ethereum') {
    try {
      const alchemy = network === 'ethereum' ? this.ethereumAlchemy : this.polygonAlchemy;
  ...
  async getTokenBalances(address: string, network: 'ethereum' | 'polygon' = 'ethereum') {
    try {
      const alchemy = network === 'ethereum' ? this.ethereumAlchemy : this.polygonAlchemy;
      ...
      const tokensWithMetadata = await Promise.all(
        balances.tokenBalances.map(async (token) => {
          ...
          return {
            ...token,
            metadata,
            formattedBalance: ethers.utils.formatUnits(token.tokenBalance || '0', metadata.decimals || 18)
          };
        })
      );
  ...
  async getTransactions(address: string, network: 'ethereum' | 'polygon' = 'ethereum') {
    try {
      const alchemy = network === 'ethereum' ? this.ethereumAlchemy : this.polygonAlchemy;
  ...
  async getNFTFloorPrice(contractAddress: string, network: 'ethereum' | 'polygon' = 'ethereum') {
    try {
      const alchemy = network === 'ethereum' ? this.ethereumAlchemy : this.polygonAlchemy;
  ...
  async getGasSpending(address: string, network: 'ethereum' | 'polygon' = 'ethereum') {
    try {
      const alchemy = network === 'ethereum' ? this.ethereumAlchemy : this.polygonAlchemy;
      const transfers = await alchemy.core.getAssetTransfers({
        fromAddress: address,
      ...
      transfers.transfers.forEach(tx => {
        if (tx.value) totalGas += Number(tx.value);
      });
